...
/*
 ...
 Purpose:     A game where the goal is to escape the prison. Every choice made in this game
 makes a difference. Certain choices can lead to death, arrest or escape. Players will move 
 their player use left and right arrows keys in order to find where the choices are. 
 ...
 interactive choices
 sound effects
 use of booleans and if statments to create multiple endings
 images
 story line of different places in the prison
 --------------------------------------------- 
...
SoundFile lose, escape;

PFont font; //allows me to set a font

PImage prisonCell, hallway, hallway2, cafeteria, doors, man, wallBlood, bloodHand, bloodHands, freedom, arrest, face; //images 
String gameState;
...
void setup() {
  ...
  //loads sound file
  lose = new SoundFile (this, "gameover.wav");
  escape = new SoundFile (this, "win.wav");
...
void draw() {
  ...
  if (gameState == "startScreen") { //start screen
    startScreen();
  } else if (gameState == "instructions") { //instruction scene
    instructions();
  } else if (gameState == "prisonCell") { //scene 1
    prisonCell();
  } else if (gameState == "hallway") { //scene 2 
    hallway();
  } else if (gameState =="cafeteria") {// scene 3
    cafeteria();
  } else if (gameState == "noItemSound") { // lose sound right before noitem scene
    lose.amp(0.1);
    lose.play();
    gameState = "noItem";
  } else if (gameState == "noItem") { //(noitem scene) concequence of choosing item that you don't have in scene 3 
    noItem();
  } else if (gameState =="tranqSound") {// lose sound right before tranq scene
    lose.amp(0.1);
    lose.play();
    gameState = "tranq";
  } else if (gameState =="tranq") {  // (tranq scene) when player picks tranquilizer in hallways and picks "tranq him" in scene 3
    tranq();
  } else if (gameState == "hallway2") { // scene 4 (after player moves past from cafeteria)
    hallway2();
  } else if (gameState == "doorsSound") { //lose sound right before doorsFriend scene
    lose.amp(0.1);
    lose.play();
    gameState = "doorsFriend";
  } else if ( gameState =="doorsFriend") { //( doors friend scene) if player decided to tell other pirsoner about escape plans
    doorsFriend();
  } else if (gameState == "doors") { // final scene
    doors();
  } else if (gameState == "escapeSound") {// win sound for when player escapes
    escape.amp(0.1);
    escape.play();
    gameState ="escape";
  } else if (gameState == "escape") { //escape scene final scene
    escape();
  } else if (gameState == "arrestSound") {// lose sound right before final arrest
    lose.amp(0.1);
    lose.play();
    gameState = "arrest";
  } else if (gameState == "arrest") {// final arrest scene
    arrest();
  }
}
...
void startScreen() {//start screen + includes start and how to play button
  image(face, -130, -200);

  fill(255); //title of game
  textSize(90);
  text("Escape The Prison", 100, 200);
  ...
  fill(255);
  textSize(50);
  text("HOW TO PLAY", 190, 620);
}

void instructions() { //instruction screen with restart button
  //box for text
  background(#640303);
  //makes spikes at top of screen 
  for ( int xTriangle = 0; xTriangle <= 800; xTriangle += 17) {
    if (xTriangle % 2 ==0) { //conidtional loops (changes colours of spikes)
      fill(#FA0F0F);
    } else { 
      fill(#810000);
    }
    triangle (xTriangle, 0, xTriangle + 10, 70, xTriangle + 20, 0);
  }

  fill(0);
  rect(50, 100, 500, 400);
  //text which explains what must be done
  fill(255);
  textSize(30);
  text("To win the game you must escape the prison", 105, 200);
  text("Your choices matter and will change the story of the game", 60, 250);
  text("use left and right arrow keys to move your player", 110, 300);
  textSize(50);
  text("good luck", 230, 450);
  restartButton();
}
...
void noItem() {// death scene occurs when player approaches prisoner and picks the item they do not have - shows series of text indicating reason for this ending
...
void tranq () { // if players decided to tranquilize prisoner  - shows series of text indicating reason for this ending 
  counter = counter +1;//increases counter
  background(0);// background turn black
  restartButton();
...
void hallway2 () { // scene 4 - displays images, text, makes choices and continues story line of the game
  ...
  if (counter < 250) { // text appears certain amount of time
    fill(255, 0, 0);
    textSize(30);
...
void doorsFriend() { // arrest scene 4.5 if player picks to bring prisoner along - displays text indicating reason for this ending
  counter = counter +1;//increases counter
  background(0);// background turn black
  restartButton(); //restart button

  if (counter < 200) {//text displays for certain time
    fill(255);
    textSize(60);
    text("Uh Oh", 250, 270);
    textSize(40);
...
void doors() { //final Scene, displays images, makes choices
  ...
  if (manX < 10) { //blocks player from moving out off screen on left
    manX = 10;
  } else if (manX > 270) {// if certain distance choices appear
    //displays button and text
    textSize(30);
    button8.one();
    fill(0);
...
void escape() { // final ending, displays images , finishis off final story line
  image(freedom, 0, 0);
  fill(11, 80, 175);
  textSize(45);
...
void arrest() { // second final ending arrest, displays images and restart, finsishes off final story line 
  background(#AF0B1E);
  image(arrest, -100, 0);
  fill(#AF0B1E);
  textSize(60);
...
void mousePressed() {
  ...
  if (gameState == "startScreen") {
    if (mouseX > 100 && mouseX < 500) { //start button
      if (mouseY > 400 && mouseY < 500) {
        gameState = "prisonCell";
      }
    }
    if (mouseX > 100 && mouseX < 500) { //how to play  button
      if (mouseY > 550 && mouseY < 600) {
        gameState = "instructions";
      }
    }
  } else if (gameState == "prisonCell") { //button1.one (open door) - prison cell scene
  ...
  else  if (gameState == "hallway2") { //if bring him picked
    if (mouseX > 240 && mouseX < 370) {
      if (mouseY > 350 && mouseY < 380) {
        gameState = "doorsSound";
        friend = true;
        manX = 10;
        counter= 0;
      }
  ...
  else if (gameState == "doors") { //if player picks door 1
    if (mouseX > 95 && mouseX < 225) {
      if (mouseY > 235 && mouseY < 275) {
        gameState = "escapeSound";
      }
    } else if (mouseX > 385 && mouseX < 515) { //if player picks door 2
      if (mouseY > 235 && mouseY < 275) {
        gameState = "arrestSound";
      }
    }
  } else if (gameState == "noItem" || gameState == "tranq" || gameState == "arrest" || gameState == "doorsFriend" || gameState == "instructions") { // for restart button in several gamestates
